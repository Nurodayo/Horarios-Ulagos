...
def nurin_scrape_horario(indice, page, carrera, browser):
    ...
    while i < cant:
        with page.context.expect_page() as new_tab:
            ...
            page.wait_for_timeout(500)  # espera 3 segundos
...
def save_json(dfl, carrera):
    carrera_nombre_a = carrera.strip()
    carrera_nombre_a = carrera_nombre_a.replace(" ", "-")
    carrera_nombre_a = carrera_nombre_a.replace("/", "-")
    df = pd.concat(dfl, ignore_index=True)
    df.to_json(str(carrera_nombre_a) + ".json", orient="records", indent=2)
    # return df
...
with sync_playwright() as p:
    ...
    dfl = nurin_scrape_horario(indice, page, i, browser)
    save_json(dfl, str(i))
    page.close()
